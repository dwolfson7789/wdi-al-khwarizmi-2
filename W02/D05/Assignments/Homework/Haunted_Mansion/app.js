...
function makeShortChoices() {
  for (var scene in story) {                // iterate over each property in the story object
    if (story.hasOwnProperty(scene)) {      // but only properties that I added to the story object
      story[scene].shortChoices = [];       // create shortChoices as an empty array in that scene
      for (var i = 0; i < story[scene].fullChoices.length; i++) {
        // I did this on one line; it's not necessary and may be easier to understand if you
        // break it into multiple commands. But to parse it out: it PUSHES into story[scene].shortChoices
        // (the new shortChoices array for the current scene in the story -- remember the for .. in loop
        // used var scene in story as the variable name). What it pushes is the current item in the
        // fullChoices array we're iterating over in the regular for loop, converted to lower case, split
        // over the space (" ") into individual words, and grabbing the first one (index 0). whew!
        story[scene].shortChoices.push(story[scene].fullChoices[i].toLowerCase().split(" ")[0]);
      }
    }
  }
  commands = story.opening.shortChoices;    // setting the commands equal to the opening's shortChoices
}
...
function speechResult() {
  recognition.onresult = function(event) {
   ...
   var words = transcript.split(" ");         // split the transcript over spaces into individual words
   var result = words[words.length - 1];      // grab the final word
   if (development) {
