...
function start() {
	// this function should call shuffle(tiles) (make sure to pass your tiles array in!) and then call makeAndDisplayTiles()

}

function makeAndDisplayTiles() {
	// needs to empty out the div (#game) that will hold the tiles. Do that here:


	// needs to clear the info text. Do that here:
	...
	// call the next function (addEventsToTiles) to add the click handlers to the tiles:
...
// !!! modify this makePlay() function -- it will need to take one argument, the title that was clicked on !!!
function makePlay(){
...
function checkForMatch() {
	...
	//use setTimeout so the innerHTML doesn't disappear immediately
...
function checkForWin() {
	...
	// (1) add a winning message to the info div (2) remove the 'found' class (3) add a 'won' class
